...
{
    public class AttributeCommandHandlers : IRequestHandler<CreateBasicAttributeCommand, int>, IRequestHandler<CreateSalesAttributeCommand, int>,
        IRequestHandler<UpdateBasicAttributeCommand>, IRequestHandler<UpdateSalesAttributeCommand>, IRequestHandler<DeleteAttributeCommand>
    {
        ...
        public async Task<int> Handle(CreateBasicAttributeCommand request, CancellationToken cancellationToken)
        {
            var attribute = Attribute.CreateBasicAttribute(request.Name, request.IsRequired, request.DisplayOrder, request.ValueType, request.ControlType, request.Description, request.IsSearch, request.UnitId, request.IsActive, request.CategoryId);
        ...
        public async Task<int> Handle(CreateSalesAttributeCommand request, CancellationToken cancellationToken)
        {
            var attribute = Attribute.CreateSalesAttribute(request.Name, request.DisplayOrder, request.Description, request.IsActive, request.CategoryId);
            if (!request.Options.IsNullOrEmpty())
            {
                foreach (var option in request.Options)
                {
        ...
        public async Task Handle(UpdateBasicAttributeCommand request, CancellationToken cancellationToken)
        {
        ...
        public async Task Handle(UpdateSalesAttributeCommand request, CancellationToken cancellationToken)
        {
            var attribute = await _attributeRepository.IncludingFirstOrDefaultAsync(request.Id, x => x.Options);
            if (attribute == null)
                throw new EntityNotFoundException(typeof(Attribute), request.Id);
